...
<template>
  <div class="bulk-view">
    <div class="page-heading"><div><h1>批量发布商品</h1><p>一次导入标准商品，分别映射并发布到微信小店和小红书</p></div><el-button :icon="Download" @click="downloadTemplate">下载模板</el-button></div>
    ...
    <section v-if="currentJob" class="content-panel bulk-card" style="margin-top:16px">
      ...
      <el-table :data="currentJob.items" max-height="320" style="margin-top:12px" :tooltip-options="{ effect: 'light', showAfter: 0, hideAfter: 0 }">
        ...
        <el-table-column label="平台" width="100"><template #default="{ row }">{{ row.platform === 'wechat' ? '微信小店' : '小红书' }}</template></el-table-column>
